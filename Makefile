...
test-e2e:
	...
	docker run -d --network calc-test-e2e --name calc-web -p 8085:80 calc-web

	docker create --network calc-test-e2e --name e2e-tests \
    -e BASE_URL="http://calc-web:80" cypress/included:4.9.0 --browser chrome
...
run-web:
	docker run --rm --volume `pwd`/web:/usr/share/nginx/html  --volume `pwd`/web/constants.local.js:/usr/share/nginx/html/constants.js --name calc-web -p 8085:80 nginx
...
deploy-stage:
	...
	docker run -d --rm --name calc-web -p 8085:80 calc-web